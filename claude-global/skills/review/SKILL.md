...
3. **Title prefix**: inspect existing issues with `gh issue list --limit 30 --json number,title` and copy the format used in practice. If the project uses sprint-planned codes like `[ISS-007a]`, `[ISS-010b]` (typically synced from an external tracker), the reviewer cannot synthesize new sprint codes. **For follow-up issues from a review**, anchor the title to the **subtask the review surfaced from** with the suffix `-followup` — e.g., a bug surfaced during PR #75 R2 (subtask 007b) gets `[ISS-007b-followup] <descriptive title>`. This mirrors the project pattern (e.g., issue #92 `[ISS-009c-followup]`) and keeps follow-ups discoverable from the parent subtask. Do not invent a parallel scheme (e.g., `[ISS-FOLLOWUP-N]`) — anchor to the subtask code that already exists.
