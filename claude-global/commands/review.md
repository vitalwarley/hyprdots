...
- **Issue↔spec scope alignment**: Compare the issue's acceptance criteria against the spec's subtask scope. If the issue lists fewer requirements than the spec, the gap is a **process failure** (issue not updated after spec enrichment), not a dev failure. Flag divergences as spec-caused in Root Cause Analysis — the dev's contract is the issue, not the spec they may never have seen. **This check is mandatory** — skipping it leads to misattributing scope gaps to the dev when the real cause is a stale issue.
...
- Don't assess code quality without first reading the spec — scope completeness and code quality are orthogonal assessments. A PR can have perfect code and still miss half the requirements. Spec-first → code-second order.
- Don't attribute scope gaps to the dev without checking their actual contract (issue body) — if the issue lists fewer requirements than the spec, the gap is process-caused (issue not synchronized), not dev negligence
- Don't mark findings resolved incrementally without re-reading Verdict + Fix Guide — partial updates create incoherent reports (empty verdicts, stale fix guides describing already-applied work). After any batch of status changes, verify the full document is self-consistent.
